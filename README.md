...
![](./images/Screenshot%202025-04-15%20at%2023.46.05.png)
...
- Anxiety Output
![](./images/anxiety_hycore.png)
![](./images/anxiety_hycore_1.png)

- Depression Output
![](./images/depression_hycore.png)
![](./images/depression_hycore_1.png)


## M3H + Visual Features (VisFuse)

![](./images/architecture.png)

# Knowledge Fusion for Multimodal Meme Analysis

Your project implements a sophisticated knowledge fusion approach for analyzing mental health memes, particularly those related to anxiety and depression. Let me explain how you're extracting and combining different types of features to create a comprehensive representation for RAG (Retrieval-Augmented Generation) during training.

## Three-Pronged Feature Extraction

Your system extracts three distinct types of information from each meme:

1. **OCR Text (Textual Content)**
   - Extracts the actual text appearing in the memes
   - Uses the BAAI/bge-m3 sentence transformer to create 1024-dimensional embeddings
   - Captures the explicit linguistic content of the meme

2. **Figurative Reasoning (Semantic Understanding)**
   - Extracts the implicit meanings, metaphors, and emotional context
   - Uses the same sentence transformer (BAAI/bge-m3) to embed this higher-level interpretation
   - This data comes from your previous extraction process using the Qwen2.5-VL-7B-Instruct model

3. **Visual Features (Image Content)**
   - Uses a modified ResNet-50 pre-trained on ImageNet
   - Removes the classification layer and adds a projection layer to match the 1024-dimension size
   - Captures visual patterns, objects, and compositional elements in the memes

## Knowledge Fusion Process

You create a unified representation through concatenation:

```
Knowledge Fusion DB = [OCR Embedding | Figurative Reasoning Embedding | Image Embedding]
```

This results in a 3072-dimensional vector (3 × 1024) for each meme, containing complementary information from all three modalities.

## Implementation Details

Your code includes several sophisticated elements:

1. **Separate Processing Pipelines** for train and test data
2. **Error Handling** for missing images or processing failures
3. **Dimension Matching** with projection layers when needed
4. **Batched Processing** for efficiency with large datasets
5. **Progress Tracking** using tqdm

## RAG Application

The resulting knowledge-fused embeddings serve as a comprehensive database for retrieval-augmented generation:

1. During training, you create and save a complete fused knowledge database
2. For test/inference, you generate individual embeddings that can be fused on-the-fly
3. This allows you to query the knowledge base with new examples by creating fused representations of those examples and finding the most similar entries in your training database

## Technical Significance

This approach is particularly valuable for meme analysis because:

1. **Multimodal Comprehension**: Memes communicate through text, imagery, and implicit meanings simultaneously
2. **Holistic Understanding**: The fused representation captures all aspects of the meme's message
3. **Complementary Information**: When one modality lacks information, the others can compensate

By integrating OCR text, figurative reasoning, and visual features into a unified representation, your system creates a rich knowledge base that captures both the explicit and implicit meanings of mental health memes, enabling more nuanced analysis and generation.

## Training Plots:

For each training, we have preprocessed the features and figurative reasoning into the dataset directory. The embeddings generated by the `embeddings_extractor.py`, then `tok_k_rag_extractor.py` generates the top k embeddings and store the csv into the dataset. Then `rag_trainer.py` trains the model on that dataset. 

1. Depression Dataset:

![](./images/m3h_training_history_multilabel_depression.png)

1. Anxiety Dataset:

![](./images/m3h_anxiety_training_history_plot_hyperparams.png)

## E2E Pipeline

Pipeline flow for the inference. Technical Architecture
...
- Reasoning Engine: Hyperbolic API (Qwen 2.5 VL 7B model)

- Classification Models: Fine-tuned transformer models for depression and anxiety analysis

- Inference Pipeline: Seamless flow from image upload to symptom detection

**Usage**

- Upload a meme image using the sidebar

- Select the desired analysis type (Depression or Anxiety)

- The system will automatically perform OCR, reasoning, and classification

- Review the results in the main panel

![Pipeline flow for the inference](./images/inference.png)

Run the application by: `streamlit run mental-health-meme-classification/src/inference/web_ui.py`

Ensure that model paths are correctly configured
