...
from transformers import BertTokenizer, BertForSequenceClassification, AdamW, DebertaV2Tokenizer, DebertaV2ForSequenceClassification
import numpy as np
import sys
import json
...
class DepressionClassifier(torch.nn.Module):
    def __init__(self, num_labels):
        ...
        self.deberta = DebertaV2ForSequenceClassification.from_pretrained("microsoft/deberta-v3-base", num_labels=num_labels)
        self.dropout = torch.nn.Dropout(0.2)  # Dropout rate of 0.2
    
    def forward(self, input_ids, attention_mask, labels=None):
        outputs = self.deberta(input_ids=input_ids, attention_mask=attention_mask, labels=labels)
        return outputs
...
if __name__ == "__main__":
    ...
    NUM_EPOCHS = 10
    LEARNING_RATE = 5e-5
    OUTPUT_DIR = os.path.join(os.path.dirname(__file__), 'output', 'ocr_deberta')
