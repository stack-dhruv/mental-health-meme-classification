...
EMBEDDING_DIR = os.path.join(project_root, "depressive_embeddings_output") # Adjust if you used a different name
...
# Validation Embeddings (individuals needed for querying)
VAL_OCR_EMB_FILE = os.path.join(EMBEDDING_DIR, "val_ocr_embeddings.pt")
VAL_FIG_EMB_FILE = os.path.join(EMBEDDING_DIR, "val_figurative_reasoning_embeddings.pt")
VAL_IMG_EMB_FILE = os.path.join(EMBEDDING_DIR, "val_image_resnet50_embeddings.pt")
...
VAL_OUTPUT_CSV_FILE = os.path.join(EMBEDDING_DIR, f"val_top_{TOP_K}_similar_indices.csv")
VAL_OUTPUT_JSON_FILE = os.path.join(EMBEDDING_DIR, f"val_top_{TOP_K}_similar_indices.json")
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Load individual validation embeddings
        val_ocr_embeddings = torch.load(VAL_OCR_EMB_FILE, map_location=DEVICE)
        val_figurative_embeddings = torch.load(VAL_FIG_EMB_FILE, map_location=DEVICE)
        val_image_embeddings = torch.load(VAL_IMG_EMB_FILE, map_location=DEVICE)

        # Load original metadata files
        train_df = pd.read_json(TARGET_TRAIN_FILE_PATH)
        test_df = pd.read_json(TARGET_TEST_FILE_PATH)
        val_df = pd.read_json(TARGET_VAL_FILE_PATH)
        print(f"Loaded {len(train_df)} train, {len(test_df)} test, and {len(val_df)} validation samples metadata.")
    ...
    if not (len(test_ocr_embeddings) == len(test_figurative_embeddings) == len(test_image_embeddings) == len(test_df)):
        ...
        sys.exit(1)
    if not (len(val_ocr_embeddings) == len(val_figurative_embeddings) == len(val_image_embeddings) == len(val_df)):
        print("Error: Mismatch in sample counts for validation data/embeddings.")
        sys.exit(1)
    ...
    val_results = process_dataset(
        finder=finder,
        ocr_embeddings=val_ocr_embeddings,
        fig_embeddings=val_figurative_embeddings,
        img_embeddings=val_image_embeddings,
        metadata_df=val_df,
        k=TOP_K,
        is_training_set=False,
    ...
    try:
        val_results_df.to_csv(VAL_OUTPUT_CSV_FILE, index=False)
        val_results_df.to_json(VAL_OUTPUT_JSON_FILE, orient='records', indent=4)
        print(f"Validation results saved successfully to:")
        print(f"  CSV: {VAL_OUTPUT_CSV_FILE}")
        print(f"  JSON: {VAL_OUTPUT_JSON_FILE}")
    except Exception as e:
